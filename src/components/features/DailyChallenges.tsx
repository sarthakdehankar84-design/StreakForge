...
import { getDailyChallenges, claimChallengeReward, type DailyChallenge } from "@/lib/challenges";
import ConfettiBurst from "@/components/features/ConfettiBurst";

interface DailyChallengesProps {
  /** Called when XP is awarded so the parent can refresh user state */
  onXPAwarded: () => void;
}

export default function DailyChallenges({ onXPAwarded }: DailyChallengesProps) {
  ...
  // Re-compute progress every 10 seconds
  useEffect(() => {
    const id = setInterval(refresh, 10_000);
    return () => clearInterval(id);
  }, [refresh]);

  // Watch for all challenges completed
  useEffect(() => {
  ...
  const handleClaim = (challenge: DailyChallenge) => {
    ...
    const xp = claimChallengeReward(challenge.id, challenge.xpReward);
    if (xp > 0) {
      refresh();
      onXPAwarded();
      toast.success(`+${xp} XP — "${challenge.title}" complete! 🎯`, {
    ...
    <>
      ...
      >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div
                ...
                style={{
                  background: "linear-gradient(135deg, rgba(147,51,234,0.3), rgba(34,211,238,0.15))",
              ...
              <div>
                <h2 className="text-sm font-display font-bold text-foreground leading-tight">Daily Challenges</h2>
                <p className="text-[9px] text-muted-foreground">Resets in {hoursLeft}h {minsLeft}m</p>
              </div>
            </div>

            {/* Progress dots + total XP */}
              ...
              <div className="flex gap-1">
                {challenges.map((c, i) => (
                  <div
                    ...
                    style={{
                      background: c.rewardClaimed
                        ? "#22c55e"
                        : c.completed
                        ? "#f59e0b"
                        : "rgba(255,255,255,0.15)",
                      boxShadow: c.completed ? (c.rewardClaimed ? "0 0 4px #22c55e" : "0 0 4px #f59e0b") : undefined,
                    }}
                  />
                ))}
              </div>
        ...
        {challenges.every((c) => c.rewardClaimed) && (
          ...
          >
            <p className="text-xs font-bold text-green-400">🎉 All challenges complete! Come back tomorrow for new ones.</p>
          </div>
        )}
      </div>
    </>
...
function ChallengeRow({
  ...
  const pct = challenge.target > 0 ? Math.round((progress / challenge.target) * 100) : 0;
  const clampedPct = Math.min(pct, 100);

  const progressColor = challenge.rewardClaimed
    ? "#22c55e"
    : challenge.completed
    ? "#f59e0b"
    : index === 0
  ...
  const progressLabel =
    challenge.type === "all_habits"
      ? challenge.completed ? "Complete" : "Incomplete"
      : challenge.type === "streak"
      ? `${progress}/${challenge.target} days`
      : challenge.type === "focus_minutes"
      ? `${progress}/${challenge.target} min`
      : challenge.type === "xp_earned"
      ? `${progress}/${challenge.target} XP`
      : challenge.type === "categories"
      ? `${progress}/${challenge.target} categories`
  ...
  return (
    <div
      ...
      style={{
        background: challenge.rewardClaimed
          ? "rgba(34,197,94,0.04)"
          : challenge.completed
      ...
      <div className="flex items-start gap-3">
        ...
        <div
          ...
          style={{
            ...
            filter: challenge.rewardClaimed ? "drop-shadow(0 0 4px rgba(34,197,94,0.4))" : undefined,
          }}
        >
          ...
          <div className="flex items-center justify-between gap-2 mb-0.5">
            <p
              className={`text-sm font-bold leading-tight ${
                challenge.rewardClaimed ? "text-green-400 line-through opacity-70" : "text-foreground"
              }`}
            >
              {challenge.title}
            </p>

            {/* XP chip / Claim button */}
            {challenge.rewardClaimed ? (
              <div className="flex items-center gap-1 flex-shrink-0">
                <CheckCircle2 className="w-3.5 h-3.5 text-green-400" />
                <span className="text-[10px] font-bold text-green-400">+{challenge.xpReward}</span>
              </div>
            ) : challenge.completed ? (
              <button
                onClick={onClaim}
                className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-black transition-all duration-200 active:scale-95 hover:opacity-90 flex-shrink-0"
                style={{
                  background: "linear-gradient(135deg, #f59e0b, #fbbf24)",
                  color: "#000",
                  boxShadow: "0 0 10px rgba(245,158,11,0.4)",
                  animation: "shield-badge-pop 0.4s ease both",
                }}
              >
                <Zap className="w-2.5 h-2.5" />
                Claim +{challenge.xpReward}
              </button>
            ) : (
              <div
              ...
              >
                <Zap className="w-2.5 h-2.5 text-forge-gold opacity-60" />
                <span className="text-[10px] font-bold text-muted-foreground">+{challenge.xpReward}</span>
              </div>
            )}
          </div>
          ...
          <div className="space-y-1">
            <div className="h-1.5 bg-white/8 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  ...
                  boxShadow: challenge.completed ? `0 0 6px ${progressColor}80` : undefined,
                }}
              />
            </div>
            <div className="flex justify-between items-center">
              ...
              <span
                ...
                style={{ color: challenge.completed ? progressColor : "rgba(255,255,255,0.3)" }}
              >
                {clampedPct}%
              </span>
            </div>
